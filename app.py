...
class Post(db.Model):
    ...
    image = db.Column(db.String(55), nullable=True)
...
class PostForm(FlaskForm):
    ...
    image = FileField('Image', validators=[FileAllowed(['jpg', 'png', 'jpeg'])], render_kw={"placeholder": "Choose an image"})
...
def admin_dashboard():
    ...
    return render_template('admin_dashboard.html', admin=admin,user= current_user)
...
def create_post():
    ...
    if form.validate_on_submit():
        ...
        post = Post(text=text, title=title, image=image , author=current_user.id)
        db.session.add(post)
        db.session.commit()
